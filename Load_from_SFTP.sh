...
sftp -i -oStrictHostKeyChecking=no "${credentials}" "${username}@${sftpServer}:${tblBiota}" || bail "Unable to retrieve ${tblBiota}"
tblBiotaAssociate="tblBiotaAssociate_${datestamp}.csv"
sftp -i -oStrictHostKeyChecking=no "${credentials}" "${username}@${sftpServer}:${tblBiotaAssociate}" || bail "Unable to retrieve ${tblBiotaAssociate}"
tblCommonName="tblCommonName_${datestamp}.csv"
sftp -i -oStrictHostKeyChecking=no "${credentials}" "${username}@${sftpServer}:${tblCommonName}" || bail "Unable to retrieve ${tblCommonName}"
